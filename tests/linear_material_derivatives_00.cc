...
{
	//u
	IndependentField<spacedim, spacedim> u("u", FE_Nothing<spacedim>(), 2, {});

	vector<DependentField<spacedim, spacedim>> dependent_fields;

	DependentField<spacedim, spacedim> u_1("u_1");
	u_1.add_term(1.0, u, 0);

	DependentField<spacedim, spacedim> u_2("u_2");
	...
	LinearMaterial00<spacedim, spacedim> linear_material(	dependent_fields,
															{},
															QGauss<spacedim>(1),
															global_data,
															C,
	...
	linear_material.compare_derivatives_with_numerical_derivatives(	e_omega,
																	e_omega_ref_sets,
																	hidden_vars,
																	x);
}
