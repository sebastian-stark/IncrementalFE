...
check(unsigned int method)
{
	...
	vector<DependentField<spacedim, spacedim>> dependent_fields(4 + 3 + 4+3*I + 10+I, DependentField<spacedim, spacedim>("q"));

	const double alpha = 1.0;

	GlobalDataIncrementalFE<spacedim> global_data;
	global_data.set_t(1.0);
	OmegaDualFluidDissipation04<spacedim> omega_dual_fluid_dissipation_04(	dependent_fields,
																			{},
																			QGauss<spacedim>(1),
																			global_data,
																			I,
	...
	omega_dual_fluid_dissipation_04.compare_derivatives_with_numerical_derivatives(	e_omega,
																					e_omega_ref_sets,
																					hidden_vars,
																					x);
}
