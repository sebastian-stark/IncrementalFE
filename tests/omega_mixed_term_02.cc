...
check(unsigned int method)
{
	...
	vector<DependentField<spacedim-1, spacedim>> dependent_fields(2, DependentField<spacedim-1, spacedim>("q"));

	const double alpha = 1.0;

	GlobalDataIncrementalFE<spacedim> global_data;
	global_data.set_t(1.0);
	OmegaMixedTerm02<spacedim> omega_mixed_term_02(	dependent_fields,
													{},
													QGauss<spacedim-1>(1),
													global_data,
													method,
	...
	omega_mixed_term_02.compare_derivatives_with_numerical_derivatives(	e_omega,
																			e_omega_ref_sets,
																			hidden_vars,
																			x,
																			n,
