...
check(unsigned int method)
{
	...
	vector<DependentField<spacedim, spacedim>> dependent_fields(4 + 4 + 3*I + 1 + I, DependentField<spacedim, spacedim>("q"));

	const double alpha = 1.0;

	GlobalDataIncrementalFE<spacedim> global_data;
	global_data.set_t(1.0);
	OmegaDualFluidDissipation01<spacedim> omega_dual_fluid_dissipation_01(	dependent_fields,
																			{},
																			QGauss<spacedim>(1),
																			global_data,
																			I,
