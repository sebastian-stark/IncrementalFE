...
#include <incremental_fe/scalar_functionals/dissipation_species_flux_01.h>
...
DissipationSpeciesFlux01<spacedim>::DissipationSpeciesFlux01(	const vector<DependentField<spacedim,spacedim>>	e_omega,
...
:
ScalarFunctional<spacedim, spacedim>(e_omega, domain_of_integration, quadrature, "DissipationSpeciesFlux01", 1, 10),
...
template<unsigned int spacedim>
bool
DissipationSpeciesFlux01<spacedim>::get_h_omega(	const Vector<double>&			e_omega,
...
const
{
	...
	if(get<0>(requested_quantities))
		h_omega = delta_t/(2.0*D*c) * i_C_i;

	if(get<1>(requested_quantities))
	{
		h_omega_1.reinit(this->e_omega.size());
		for(unsigned int m = 0; m < 3; ++m)
			h_omega_1[m] = 1.0/(D*c) * C_i[m];
	}
	...
	if(get<2>(requested_quantities))
	{
		...
		for(unsigned int m = 0; m < 3; ++m)
			for(unsigned int n = 0; n < 3; ++n)
				h_omega_2(m, n) = 1.0/(delta_t*D*c) * C[m][n];
	}
...
template<unsigned int spacedim>
double
DissipationSpeciesFlux01<spacedim>::get_maximum_step(	const Vector<double>& 			e_omega,
														const vector<Vector<double>>&	/*e_omega_ref_sets*/,
...
template class DissipationSpeciesFlux01<2>;
template class DissipationSpeciesFlux01<3>;
