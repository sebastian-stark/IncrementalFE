#include <incremental_fe/scalar_functionals/initial_osmotic_pressure_00.h>
...
void
InitialOsmoticPressure00<spacedim>::compute_derivatives(const Vector<double>&			e_omega,
														double&							val,
														Vector<double>&					d1,
	...
	if(get<2>(requested_quantities))
	{
		...
		for(unsigned int m = 0; m < 9; ++m)
			for(unsigned int n = 0; n < 9; ++n)
				d2(m, n) = -p_0 * d2J_dF2(m,n);
	}
}

template<unsigned int spacedim>
InitialOsmoticPressure00<spacedim>::InitialOsmoticPressure00(	const vector<DependentField<spacedim,spacedim>>	e_omega,
									...
									const double									alpha)
:
ScalarFunctional<spacedim, spacedim>(e_omega, domain_of_integration, quadrature, "InitialOsmoticPressure00", 1),
...
template<unsigned int spacedim>
bool
InitialOsmoticPressure00<spacedim>::get_h_omega(	const Vector<double>&			e_omega,
...
template class InitialOsmoticPressure00<2>;
template class InitialOsmoticPressure00<3>;
